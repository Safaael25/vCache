"""
Runs mixed_cost_recency_workload.json across cache sizes = 10/20/30/40/50/60/70%
...
EVICTION_PCT = 0.5
N_CLUSTERS = 20
PCTS = [0.10, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70]
CACHE_SIZES = sorted({round(p * N_CLUSTERS) for p in PCTS})
COST_FIELD = "response_gpt-4o-mini_lat"

dataset_path = os.path.join(os.path.dirname(__file__), "mixed_cost_recency_workload.json")
...
bur_indices = [i for i, row in enumerate(workload) if row["tier"] == "bursty"]
fil_indices = [i for i, row in enumerate(workload) if row["tier"] == "filler"]
...
results_dir = os.path.join(os.path.dirname(__file__), "mixed_cost_recency_sweep_results")
...
for max_size in CACHE_SIZES:
    for policy_name, make_policy in make_policies(max_size).items():
        ...
        row_result = {
            ...
            "bursty_hit_rate_pct": round(hit_rate(hits, bur_indices), 1),
            "filler_hit_rate_pct": round(hit_rate(hits, fil_indices), 1),
...
print("=== FULL RESULTS: mixed_cost_recency_workload.json ===")
for max_size in CACHE_SIZES:
